...
* Außerdem müssen die folgenden Umgebungsvariablen gesetzt werden:
	`DOCKER_PWD`
...
Als nächstes müssen die Container mit Maven gebildet werden. Hierfür sollte der Befehl `` bash mvn clean package docker:build`` ausgeführt werden.

Um die Container Images auf Docker Hub zu pushen muss ``bash mvn clean package docker:build docker:push`` verwendet werden.

Schließlich muss im Projekt-Verzeichnis noch ``bash docker-compose up -d`` ausgeführt werden, um die in der `docker-compose.yml` aufgelisteten Container zu starten.
